@extends('layouts.main')
@section('content')

<div style="background: rgb(107,211,210);
background: linear-gradient(90deg, rgba(107,211,210,1) 0%, rgba(214,0,255,1) 100%);">
    <div class="container jumbothing">
        <div id="carouselExampleIndicators" class="carousel slide" data-bs-ride="true">
            <div class="carousel-inner mx-auto text-white" style="width: 90%">

                <div class="carousel-item active">
                    <div class="row align-items-center carousel-h-fxd">
                        <div class="col-lg-6 mb-2 mt-2 ml-5">
                            <div class="display-4">
                                Pelajari Lebih Lanjut Tentang <b>Indo Survei</b>
                            </div>
                            <button class="btn btn-dark mt-4">Cek Artikel</button>
                        </div>
                        <div class="col-lg-6 mb-2 mt-2 text-center">
                            <img class="carousel-img-w" src="{{url('/')}}/assets/illustration/banner1.png" alt="">
                        </div>
                    </div>
                </div>

                <div class="carousel-item">
                    <div class="row align-items-center carousel-h-fxd">
                        <div class="col-lg-6 mb-2 mt-2 ml-5">
                            <div class="display-4">
                                Buat Survey jadi <b>Lebih Mudah</b>
                            </div>
                            <button class="btn btn-dark mt-4">Mulai buat surveimu, Gratis!</button>
                        </div>
                        <div class="col-lg-6 mb-2 mt-2 text-center">
                            <img class="carousel-img-w" src="{{url('/')}}/assets/illustration/chart_indosurvei.png"
                                alt="">
                        </div>
                    </div>
                </div>

                <div class="carousel-item">
                    <div class="row align-items-center carousel-h-fxd">
                        <div class="col-lg-6 mb-2 mt-2 ml-5">
                            <div class="display-4">
                                Template Survei <b>Indo Survei</b>
                            </div>
                            <button class="btn btn-dark mt-4">Cek Template Kami</button>
                        </div>
                        <div class="col-lg-6 mb-2 mt-2 text-center">
                            <img class="carousel-img-w" src="{{url('/')}}/assets/illustration/img-survei.png" alt="">
                        </div>
                    </div>
                </div>

            </div>
            <button class="carousel-control-prev" type="button" data-bs-target="#carouselExampleIndicators"
                data-bs-slide="prev" style="width: auto">
                <i class="fa-solid fa-chevron-left text-dark fs-2"></i>
            </button>
            <button class="carousel-control-next" type="button" data-bs-target="#carouselExampleIndicators"
                data-bs-slide="next" style="width: auto">
                <i class="fa-solid fa-chevron-right text-dark fs-2"></i>
            </button>
            <div class="carousel-indicators mt-4" style="position: relative; margin-bottom: 0px;">
                <button type="button" data-bs-target="#carouselExampleIndicators" data-bs-slide-to="0" class="active"
                    aria-current="true" aria-label="Slide 1"></button>
                <button type="button" data-bs-target="#carouselExampleIndicators" data-bs-slide-to="1"
                    aria-label="Slide 2"></button>
                <button type="button" data-bs-target="#carouselExampleIndicators" data-bs-slide-to="2"
                    aria-label="Slide 3"></button>
            </div>
        </div>
    </div>
</div>
{{-- <div style="width: 100%; height: 50px; background: #000;"></div> --}}

<section class="sect-bg">
    <div class="container" style="padding-top: 1rem; padding-bottom: 1rem;">
        <div class="text-center">
            <img style="width: 200px; margin-bottom: 2rem;" src="{{url('/')}}/assets/logo/logo.png" alt="">
        </div>
        <div class="mb-5">
            <h2 class="mb-4">
                <b class="text-warm">Mengapa</b> Survey itu Penting?
            </h2>
            <p>
                Berdasarkan penelitian, 42% perusahaan baru gagal karena produk yang mereka tawarkan tidak sesuai dengan
                kebutuhan pelanggan. Sebagian besar dari mereka belum memvalidasi bisnis/produk perusahaan baru mereka
                ke
                target pasar, sehingga menyebabkan bisnis tidak berjalan dengan baik dan kurang mendapat perhatian. Dan
                kabar baiknya, hal-hal tersebut bisa dihindari dengan melakukan survei terlebih dahulu.
            </p>
            <p>
                Survei akan membantu perusahaan baru untuk mendapatkan & menemukan informasi yang berguna untuk kemajuan
                bisnis/perusahaan baru, seperti mengetahui kebutuhan, kepuasan, keinginan pelanggan untuk bisnis mereka,
                termasuk apa yang kurang dari bisnis dan apa yang dapat ditingkatkan.
            </p>
        </div>
        <div class="mb-5">
            <h2 class="mb-4">
                <b class="text-warm">Apa</b> itu Indo Survei?
            </h2>
            <p>
                Indo Survei adalah aggregator teknologi dalam bidang consumer loyalty yang dapat mengukur indeks
                kepuasan pelanggan dari berbagai sektor baik barang ataupun jasa.
            </p>
            <p>
                Tidak hanya itu, Indo Survei juga dapat membantu Anda untuk mendapatkan feedback dari pelanggan,
                mengukur kepuasan karyawan, kuis untuk anak sekolah, atau riset untuk skripsi Anda.
            </p>
            <p>
                Dengan tampilan yang menarik. mudah digunakan dan didukung oleh banyak fitur untuk survei Anda seperti
                bank soal, pasar template survei responden reward, dan dasbor visualisasi untuk membantu pengguna
                melakukan survei secara efisien.
            </p>
        </div>
    </div>
</section>

<section>
    <div class="container pt-5 pb-5">
        <div class="row">
            <div class="col-md-5 mb-4 text-center">
                <img class="carousel-img-w" src="{{url('/')}}/assets/illustration/chart_indosurvei.png" alt="">
            </div>
            <div class="col-md-7">
                <h2 class="mb-4">
                    Alat Survei yang Tepat Untuk Anda
                </h2>
                <p>
                    Indo Survei adalah platform yang dapat digunakan pengguna untuk melakukan survei melalui internet
                    dengan mudah dan cepat.
                </p>
                <p>
                    Pengguna dapat menjalankan survei kepuasan pelanggan untuk mendapatkan umpan balik tentang produk
                    dan layanan, dan mereka dapat mengirim survei keterlibatan karyawan untuk memahami kepuasan karyawan
                    di dalam perusahaan.
                </p>
                <p>
                    Fitur-fitur ini sangat memudahkan Anda untuk melakukan survei!
                </p>
            </div>
        </div>
    </div>
</section>

<section class="sect-bg">
    <div class="container pt-5 pb-5">
        <style>
            .cards {
                position: relative;
                height: 275px;
                margin: 0 auto;
                background: #fff;
                border-radius: 15px;
                margin-top: 10px;
                margin-bottom: 10px;
            }

            .cards .face {
                position: absolute;
                bottom: 0;
                left: 0;
                width: 100%;
                height: 100%;
                display: flex;
                justify-content: center;
                align-items: center;
            }

            .cards .face1 {
                box-sizing: border-box;
                padding: 20px;
                align-items: start;
            }

            .cards h2 {
                margin: 0;
                padding: 0;
            }

            .cards .face2 {
                transition: 0.5s;
            }

            .cards h2 {
                margin: 0;
                padding: 0;
                color: #fff;
                text-shadow: 0 2px 5px rgba(0, 0, 0, 0.2);
                z-index: 10;
                transition: 0.5s ease;
                font-size: 1.25em;
            }

            .cards svg {
                margin-right: 5%;
                padding: 0;
                color: #fff;
                text-shadow: 0 2px 5px rgba(0, 0, 0, 0.2);
                z-index: 10;
                transition: 0.5s ease;
            }

            .cards:hover .face.face2 {
                height: 60px;
            }

            .cards:hover .face.face2 h2 {
                font-size: 1em;
                transition: 0.5s ease;
            }

            .cards:hover .face.face2 svg {
                width: 35px;
                margin-right: 5px;
                transition: 0.5s ease;
            }

            .cards .face.face2 {
                background: #61481c;
                border-radius: 15px;
            }

        </style>

        <div class="pt-5 mb-5 justify-content-center row desktop">
            <div class="col-md-12 text-center">
                <h3 class="text-brown" style="font-weight: 700">4 Roles Dalam Penyelesaian Sengketatanah.id</h3>
            </div>


            <div class="col-md-3">
                <div class="cards">
                    <div class="face face1">
                        <div class="content">
                            <span class="stars"></span>
                            <h2 class="java text-brown">Reward Responden</h2>
                            <p class="java">Anda bisa memberikan reward bagi responden yang telah mengisi survei Anda
                                dengan mudah dan cepat.</p>
                        </div>
                    </div>
                    <div class="face face2">
                        <svg xmlns="http://www.w3.org/2000/svg" version="1.1" xmlns:xlink="http://www.w3.org/1999/xlink"
                            xmlns:svgjs="http://svgjs.com/svgjs" width="75px" x="0" y="0" viewBox="0 0 512 512.001"
                            style="enable-background:new 0 0 512 512" xml:space="preserve" class="">
                            <g>
                                <path
                                    d="m233.035156 460.980469v-10.304688c0-.246093-.011718-.496093-.035156-.746093-4.003906-40.019532-21.5-66.460938-50.578125-76.453126l-27.570313-9.566406-49.550781-42.066406c-7.542969-6.40625-18.945312-5.949219-25.960937 1.027344-6.300782 6.277344-7.371094 15.84375-3.03125 23.269531l-3.988282-1.835937-22.425781-27.03125-10.15625-25.828126c-3.699219-9.410156-13.921875-14.796874-23.773437-12.523437-5.761719 1.324219-10.675782 5.066406-13.488282 10.269531-2.8125 5.199219-3.25 11.363282-1.203124 16.910156l11.578124 31.394532c.191407.523437.445313 1.023437.75 1.488281l30.972657 47.671875c.175781.269531.367187.523438.574219.769531 10.738281 12.636719 24.289062 22.949219 39.1875 29.828125 12.296874 5.675782 30.003906 14.691406 41.742187 27.378906 3.015625 3.257813 5.882813 7.144532 7.886719 17.558594-4.402344 2.597656-7.363282 7.390625-7.363282 12.859375v22.03125c0 8.226563 6.691407 14.917969 14.917969 14.917969h86.550781c8.226563 0 14.917969-6.691406 14.917969-14.917969v-22.03125c0-6.484375-4.160156-12.019531-9.953125-14.070312zm-142.414062-57.347657c-12.792969-5.90625-24.449219-14.738281-33.738282-25.5625l-30.25-46.554687-11.289062-30.605469c-.757812-2.058594-.121094-3.757812.328125-4.585937s1.519531-2.292969 3.660156-2.785157c2.667969-.617187 5.445313.84375 6.449219 3.394532l10.59375 26.945312c.292969.738282.699219 1.429688 1.207031 2.042969l24.28125 29.269531c.714844.863282 1.617188 1.554688 2.636719 2.023438l37.191406 17.121094 19.609375 22.308593c1.480469 1.683594 3.554688 2.546875 5.636719 2.546875 1.757812 0 3.523438-.617187 4.949219-1.867187 3.109375-2.734375 3.414062-7.472657.679687-10.585938l-20.660156-23.5c-.046875-.054687-.097656-.109375-.148438-.160156l-21.980468-23.792969c-1.507813-1.660156-1.441406-4.199218.148437-5.78125 1.527344-1.523437 4.019531-1.625 5.667969-.226562l50.621094 42.976562c.707031.597656 1.519531 1.0625 2.394531 1.363282l28.917969 10.035156c23.398437 8.042968 37.027344 29.375 40.511718 63.402344v9.078124h-69.203124c-2.464844-13.292968-6.523438-20.042968-11.746094-25.691406-13.550782-14.640625-33.011719-24.59375-46.46875-30.808594zm137.367187 93.367188h-86.386719v-21.867188h86.386719zm0 0"
                                    fill="#ffffff" data-original="#000000" class=""></path>
                                <path
                                    d="m494.542969 289.191406c-2.808594-5.203125-7.726563-8.945312-13.488281-10.269531-9.851563-2.273437-20.074219 3.113281-23.773438 12.523437l-10.15625 25.824219-22.425781 27.035157-3.96875 1.828124c4.324219-7.398437 3.257812-16.980468-3.050781-23.261718-7.011719-6.980469-18.417969-7.433594-25.957032-1.03125l-49.554687 42.066406-27.546875 9.5625c-29.101563 10-46.597656 36.4375-50.597656 76.460938-.027344.246093-.039063.496093-.039063.746093v10.304688c-5.792969 2.050781-9.953125 7.582031-9.953125 14.070312v22.027344c0 8.226563 6.695312 14.917969 14.917969 14.917969h86.550781c8.226562 0 14.917969-6.691406 14.917969-14.917969v-22.027344c0-5.472656-2.960938-10.265625-7.359375-12.863281 2-10.414062 4.871094-14.300781 7.882812-17.558594 11.738282-12.6875 29.445313-21.699218 41.742188-27.378906 14.898437-6.878906 28.453125-17.191406 39.1875-29.828125.210937-.242187.402344-.5.574218-.769531l30.972657-47.667969c.304687-.46875.558593-.96875.75-1.492187l11.578125-31.390626c2.046875-5.546874 1.609375-11.710937-1.203125-16.910156zm-139.125 207.808594h-86.386719v-21.867188h86.386719zm126.257812-196.089844-11.289062 30.605469-30.25 46.554687c-9.289063 10.824219-20.945313 19.65625-33.738281 25.5625-13.457032 6.214844-32.914063 16.164063-46.464844 30.8125-5.222656 5.644532-9.28125 12.394532-11.746094 25.6875h-69.203125v-9.074218c3.484375-34.03125 17.113281-55.363282 40.535156-63.410156l28.890625-10.027344c.878906-.304688 1.691406-.769532 2.398438-1.367188l50.621094-42.976562c1.648437-1.394532 4.136718-1.296875 5.667968.226562 1.589844 1.582032 1.65625 4.125.199219 5.730469l-22.03125 23.839844c-.046875.054687-.097656.109375-.148437.164062l-20.660157 23.5c-2.734375 3.113281-2.429687 7.851563.683594 10.585938 1.421875 1.253906 3.1875 1.867187 4.949219 1.867187 2.078125 0 4.152344-.859375 5.632812-2.546875l19.609375-22.304687 37.191407-17.121094c1.019531-.472656 1.921874-1.160156 2.636718-2.027344l24.28125-29.269531c.507813-.609375.914063-1.300781 1.207032-2.042969l10.59375-26.941406c1.003906-2.554688 3.777343-4.015625 6.449218-3.398438 2.140625.492188 3.210938 1.957032 3.660156 2.785157.449219.828125 1.082032 2.527343.324219 4.585937zm0 0"
                                    fill="#ffffff" data-original="#000000" class=""></path>
                                <path
                                    d="m373.320312 134.125 16.421876 13.109375-5.578126 20.257813c-1.183593 4.296874.382813 8.761718 3.984376 11.382812 1.871093 1.355469 4.042968 2.039062 6.222656 2.039062 2.019531 0 4.042968-.585937 5.832031-1.765624l17.542969-11.5625 17.546875 11.5625c3.71875 2.449218 8.449219 2.34375 12.054687-.273438 3.601563-2.617188 5.167969-7.085938 3.984375-11.382812l-5.578125-20.257813 16.421875-13.113281c3.480469-2.777344 4.839844-7.308594 3.464844-11.546875-1.378906-4.234375-5.144531-7.101563-9.589844-7.308594l-20.992187-.953125-7.398438-19.671875c-1.566406-4.167969-5.457031-6.863281-9.914062-6.863281-4.453125 0-8.34375 2.695312-9.914063 6.863281l-7.394531 19.671875-20.992188.953125c-4.449218.203125-8.214843 3.074219-9.589843 7.308594-1.375 4.238281-.015625 8.769531 3.464843 11.550781zm32.742188-5.054688c2.996094-.136718 5.621094-2.042968 6.675781-4.851562l5.011719-13.320312 5.007812 13.320312c1.054688 2.808594 3.683594 4.714844 6.679688 4.851562l14.214844.648438-11.121094 8.878906c-2.34375 1.871094-3.347656 4.960938-2.550781 7.851563l3.777343 13.722656-11.882812-7.832031c-1.25-.824219-2.6875-1.238282-4.125-1.238282s-2.878906.414063-4.128906 1.238282l-11.882813 7.832031 3.777344-13.722656c.796875-2.890625-.207031-5.980469-2.550781-7.851563l-11.121094-8.878906zm0 0"
                                    fill="#ffffff" data-original="#000000" class=""></path>
                                <path
                                    d="m34.84375 134.125 16.421875 13.109375-5.578125 20.261719c-1.183594 4.292968.382812 8.761718 3.984375 11.378906 3.605469 2.617188 8.335937 2.722656 12.054687.273438l17.546876-11.5625 17.542968 11.5625c1.789063 1.179687 3.8125 1.765624 5.832032 1.765624 2.179687 0 4.351562-.683593 6.222656-2.039062 3.605468-2.621094 5.167968-7.085938 3.984375-11.382812l-5.578125-20.257813 16.421875-13.109375c3.480469-2.78125 4.839843-7.3125 3.464843-11.550781-1.375-4.234375-5.140624-7.101563-9.589843-7.308594l-20.992188-.953125-7.394531-19.671875c-1.570312-4.167969-5.460938-6.863281-9.914062-6.863281-4.457032 0-8.347657 2.695312-9.914063 6.863281l-7.394531 19.671875-20.992188.953125c-4.453125.203125-8.214844 3.074219-9.589844 7.308594-1.378906 4.238281-.019531 8.769531 3.460938 11.550781zm32.738281-5.054688c2.996094-.136718 5.625-2.042968 6.679688-4.851562l5.011719-13.320312 5.007812 13.320312c1.054688 2.808594 3.683594 4.714844 6.679688 4.851562l14.214843.648438-11.121093 8.878906c-2.34375 1.871094-3.347657 4.960938-2.550782 7.851563l3.777344 13.722656-11.882812-7.832031c-2.503907-1.652344-5.75-1.652344-8.253907 0l-11.882812 7.832031 3.777343-13.722656c.796876-2.890625-.207031-5.980469-2.550781-7.851563l-11.121093-8.878906zm0 0"
                                    fill="#ffffff" data-original="#000000" class=""></path>
                                <path
                                    d="m204.082031 46.34375 16.421875 13.113281-5.578125 20.257813c-1.183593 4.296875.382813 8.761718 3.984375 11.378906 3.601563 2.617188 8.335938 2.726562 12.054688.273438l17.546875-11.5625 17.542969 11.566406c1.789062 1.179687 3.8125 1.765625 5.832031 1.765625 2.179687 0 4.351562-.683594 6.222656-2.042969 3.605469-2.617188 5.167969-7.082031 3.984375-11.378906l-5.578125-20.257813 16.421875-13.113281c3.480469-2.777344 4.839844-7.3125 3.464844-11.546875-1.375-4.238281-5.140625-7.105469-9.589844-7.308594l-20.992188-.957031-7.398437-19.667969c-1.566406-4.167969-5.457031-6.863281-9.910156-6.863281-4.457031 0-8.347657 2.695312-9.914063 6.863281l-7.398437 19.667969-20.992188.957031c-4.449219.203125-8.214843 3.070313-9.589843 7.308594-1.375 4.234375-.015626 8.769531 3.464843 11.546875zm32.742188-5.050781c2.996093-.136719 5.621093-2.046875 6.675781-4.855469l5.011719-13.320312 5.007812 13.320312c1.054688 2.808594 3.683594 4.71875 6.679688 4.855469l14.214843.644531-11.121093 8.882812c-2.34375 1.871094-3.347657 4.957032-2.550781 7.851563l3.777343 13.722656-11.878906-7.835937c-1.253906-.824219-2.691406-1.238282-4.128906-1.238282s-2.875.414063-4.128907 1.238282l-11.882812 7.835937 3.777344-13.722656c.796875-2.894531-.207032-5.980469-2.550782-7.851563l-11.121093-8.882812zm0 0"
                                    fill="#ffffff" data-original="#000000" class=""></path>
                                <path
                                    d="m375.097656 238.234375c0-69.800781-56.789062-126.589844-126.585937-126.589844-69.800781 0-126.589844 56.789063-126.589844 126.589844 0 69.796875 56.789063 126.585937 126.589844 126.585937 69.796875 0 126.585937-56.789062 126.585937-126.585937zm-126.585937 111.585937c-61.53125 0-111.589844-50.058593-111.589844-111.585937 0-61.53125 50.058594-111.589844 111.589844-111.589844 61.527343 0 111.585937 50.058594 111.585937 111.589844 0 61.527344-50.058594 111.585937-111.585937 111.585937zm0 0"
                                    fill="#ffffff" data-original="#000000" class=""></path>
                                <path
                                    d="m156.542969 230.636719c4.082031.667969 7.945312-2.101563 8.617187-6.191407 3.203125-19.523437 13.292969-37.417968 28.410156-50.375 15.277344-13.09375 34.789063-20.304687 54.941407-20.304687 20.148437 0 39.664062 7.210937 54.9375 20.304687 15.117187 12.957032 25.207031 30.851563 28.410156 50.375.605469 3.679688 3.785156 6.289063 7.390625 6.289063.40625 0 .816406-.03125 1.226562-.101563 4.085938-.667968 6.855469-4.527343 6.183594-8.613281-3.773437-23.011719-15.652344-44.082031-33.445312-59.335937-17.996094-15.425782-40.976563-23.917969-64.703125-23.917969-23.730469 0-46.710938 8.492187-64.703125 23.917969-17.796875 15.253906-29.675782 36.324218-33.449219 59.335937-.671875 4.085938 2.097656 7.945313 6.183594 8.617188zm0 0"
                                    fill="#ffffff" data-original="#000000" class=""></path>
                                <path
                                    d="m340.476562 245.832031c-4.085937-.671875-7.945312 2.097657-8.617187 6.1875-3.203125 19.527344-13.292969 37.417969-28.410156 50.375-15.277344 13.09375-34.789063 20.308594-54.9375 20.308594-20.152344 0-39.664063-7.214844-54.941407-20.308594-15.117187-12.957031-25.207031-30.847656-28.410156-50.375-.671875-4.085937-4.53125-6.859375-8.617187-6.1875-4.085938.671875-6.855469 4.527344-6.183594 8.613281 3.773437 23.011719 15.652344 44.082032 33.449219 59.335938 17.992187 15.425781 40.972656 23.917969 64.703125 23.917969 23.726562 0 46.707031-8.492188 64.703125-23.917969 17.792968-15.253906 29.671875-36.324219 33.445312-59.335938.671875-4.085937-2.097656-7.941406-6.183594-8.613281zm0 0"
                                    fill="#ffffff" data-original="#000000" class=""></path>
                                <path
                                    d="m249.828125 284.203125c1.886719 0 3.769531-1.382813 3.769531-2.890625v-4.269531c13.566406-1.886719 22.863282-10.429688 22.863282-25.878907 0-17.085937-11.933594-22.613281-22.863282-26.632812v-25.503906c8.792969.628906 12.058594 4.648437 15.074219 4.648437 3.769531 0 5.527344-4.773437 5.527344-7.160156 0-6.15625-12.058594-8.792969-20.601563-9.046875v-3.390625c0-1.507813-1.886718-2.890625-3.769531-2.890625-2.136719 0-3.644531 1.382812-3.644531 2.890625v3.644531c-11.933594 1.253906-23.867188 7.535156-23.867188 23.363282 0 16.082031 12.5625 20.605468 23.867188 24.625v29.519531c-12.8125-1.003907-16.207032-9.796875-20.351563-9.796875-3.140625 0-5.777343 4.144531-5.777343 7.160156 0 6.15625 10.550781 14.570312 26.128906 14.824219v3.894531c.003906 1.507812 1.511718 2.890625 3.644531 2.890625zm3.015625-45.726563c6.15625 2.636719 11.054688 6.15625 11.054688 14.070313 0 7.15625-4.269532 11.179687-11.054688 12.433594zm-17.964844-28.644531c0-6.53125 5.023438-9.671875 12.0625-10.554687v22.867187c-6.660156-2.511719-12.0625-5.152343-12.0625-12.3125zm0 0"
                                    fill="#ffffff" data-original="#000000" class=""></path>
                            </g>
                        </svg>
                        <h2>Reward Responden</h2>
                    </div>
                </div>
            </div>
            <div class="col-md-3">
                <div class="cards">
                    <div class="face face1">
                        <div class="content">
                            <span class="stars"></span>
                            <h2 class="java text-brown">Survey Template Market</h2>
                            <p class="java">Pilih dengan harga terjangkau untuk semua kualitas template survei di Indo
                                Survei.</p>
                        </div>
                    </div>
                    <div class="face face2">
                        <svg xmlns="http://www.w3.org/2000/svg" version="1.1" xmlns:xlink="http://www.w3.org/1999/xlink"
                            xmlns:svgjs="http://svgjs.com/svgjs" width="75px" x="0" y="0" viewBox="0 0 64 64"
                            style="enable-background:new 0 0 512 512" xml:space="preserve" class="">
                            <g>
                                <path
                                    d="M22,23H15V16a1,1,0,0,0-1-1,9,9,0,1,0,9,9A1,1,0,0,0,22,23Zm-9-5.929v6.515l-4.6,4.6A6.985,6.985,0,0,1,13,17.071ZM14,31a6.953,6.953,0,0,1-4.184-1.4l4.6-4.6h6.515A7.011,7.011,0,0,1,14,31Z"
                                    fill="#ffffff" data-original="#000000" class=""></path>
                                <path
                                    d="M18,21h8a1,1,0,0,0,1-1,9.011,9.011,0,0,0-9-9,1,1,0,0,0-1,1v8A1,1,0,0,0,18,21Zm1-7.929A7.018,7.018,0,0,1,24.929,19H19Z"
                                    fill="#ffffff" data-original="#000000" class=""></path>
                                <path
                                    d="M39,26A13,13,0,1,0,52,39,13.015,13.015,0,0,0,39,26Zm-5,9V29.214a10.974,10.974,0,0,1,2-.788V35Zm12,2v2H32V37Zm-4-2V28.426a10.974,10.974,0,0,1,2,.788V35Zm-2-6.949V35H38V28.051c.33-.03.662-.051,1-.051S39.67,28.021,40,28.051Zm-8,2.47V35H31a1,1,0,0,0-1,1v4a1,1,0,0,0,1,1H47a1,1,0,0,0,1-1V36a1,1,0,0,0-1-1H46V30.521A10.948,10.948,0,0,1,49.236,43H28.764A10.948,10.948,0,0,1,32,30.521ZM39,50a11,11,0,0,1-9.208-5H48.208A11,11,0,0,1,39,50Z"
                                    fill="#ffffff" data-original="#000000" class=""></path>
                                <path
                                    d="M61.671,55.257l-4.326-4.326A2,2,0,0,0,57,48.586l-2.545-2.545A16.883,16.883,0,0,0,53,29.378V4a3,3,0,0,0-3-3H4A3,3,0,0,0,1,4V42a3,3,0,0,0,3,3H23.114a16.934,16.934,0,0,0,22.927,9.455L48.586,57a1.972,1.972,0,0,0,2.345.345l4.326,4.326a4.535,4.535,0,1,0,6.414-6.414ZM4,3H50a1,1,0,0,1,1,1V7H3V4A1,1,0,0,1,4,3ZM4,43a1,1,0,0,1-1-1V9H51V26.971a17.056,17.056,0,0,0-5-3.445V12a1,1,0,0,0-1-1H41a1,1,0,0,0-1,1V22.051c-.333-.02-.662-.051-1-.051s-.667.031-1,.051V18a1,1,0,0,0-1-1H33a1,1,0,0,0-1,1v5.526A16.882,16.882,0,0,0,22.5,43ZM44,22.753a16.817,16.817,0,0,0-2-.472V13h2Zm-8-.472a16.817,16.817,0,0,0-2,.472V19h2ZM24,39A15,15,0,1,1,39,54,15.017,15.017,0,0,1,24,39ZM47.882,53.468a17.1,17.1,0,0,0,5.586-5.586L55.586,50,50,55.586ZM58.464,61a2.52,2.52,0,0,1-1.793-.743L52.414,56,56,52.414l4.257,4.257A2.536,2.536,0,0,1,58.464,61Z"
                                    fill="#ffffff" data-original="#000000" class=""></path>
                                <path
                                    d="M18,35H6a1,1,0,0,0-1,1v4a1,1,0,0,0,1,1H18a1,1,0,0,0,1-1V36A1,1,0,0,0,18,35Zm-1,4H7V37H17Z"
                                    fill="#ffffff" data-original="#000000" class=""></path>
                            </g>
                        </svg>
                        <h2 style="font-size: 1em">Survey Template Market</h2>
                    </div>
                </div>
            </div>
            <div class="col-md-3">
                <div class="cards">
                    <div class="face face1">
                        <div class="content">
                            <span class="stars"></span>
                            <h2 class="java text-brown">Bank Pertanyaan</h2>
                            <p class="java">Ada Bank Soal yang bisa ditambahkan ke survei Anda dengan cepat & efisien.
                            </p>
                        </div>
                    </div>
                    <div class="face face2">
                        <svg xmlns="http://www.w3.org/2000/svg" version="1.1" xmlns:xlink="http://www.w3.org/1999/xlink"
                            xmlns:svgjs="http://svgjs.com/svgjs" width="85px" x="0" y="0" viewBox="0 0 64 64"
                            style="enable-background:new 0 0 512 512" xml:space="preserve" class="">
                            <g>
                                <g>
                                    <g id="LWPOLYLINE">
                                        <g>
                                            <path
                                                d="m31.321 38.904c-.006 0-.012 0-.018 0-1.104-.01-1.992-.913-1.982-2.018l.009-1c0-.057.003-.115.009-.172.277-2.909 2.208-4.692 3.761-6.125.526-.486 1.024-.945 1.448-1.424.517-.583 1.267-1.777.478-3.218-.909-1.664-3.129-2.135-4.854-1.74-1.802.412-2.466 1.951-2.702 2.831-.286 1.067-1.383 1.7-2.45 1.415-1.067-.286-1.7-1.383-1.415-2.45.797-2.977 2.865-5.052 5.673-5.695 3.779-.863 7.585.664 9.256 3.719 1.391 2.544 1.01 5.529-.995 7.791-.557.628-1.152 1.178-1.728 1.709-1.435 1.325-2.332 2.211-2.484 3.49l-.008.903c-.008 1.101-.902 1.984-1.998 1.984z"
                                                fill="#ffffff" data-original="#000000" class=""></path>
                                        </g>
                                    </g>
                                    <g id="POINT">
                                        <g>
                                            <path
                                                d="m31.32 44.9c-.52 0-1.04-.21-1.41-.58-.38-.37-.59-.89-.59-1.42 0-.52.21-1.04.59-1.41.74-.74 2.08-.74 2.82 0 .38.37.59.89.59 1.41 0 .53-.21 1.04-.58 1.42-.38.37-.89.58-1.42.58z"
                                                fill="#ffffff" data-original="#000000" class=""></path>
                                        </g>
                                    </g>
                                    <g id="CIRCLE_2_">
                                        <g>
                                            <path
                                                d="m31.923 57.997c-2.275 0-4.53-.296-6.703-.881-9.364-2.519-16.696-10.276-18.679-19.763-2.021-9.665 1.709-19.825 9.504-25.884 4.506-3.503 10.189-5.433 16.003-5.433 4.025 0 8.022.936 11.56 2.705 8.737 4.371 14.382 13.503 14.382 23.266 0 7.966-3.759 15.644-10.055 20.538-4.522 3.516-10.209 5.452-16.012 5.452zm.126-47.961c-4.929 0-9.741 1.63-13.548 4.591-6.597 5.128-9.755 13.727-8.044 21.907 1.678 8.025 7.881 14.588 15.803 16.719 1.834.493 3.74.744 5.663.744h.001c4.918 0 9.732-1.637 13.557-4.61 5.329-4.142 8.51-10.639 8.51-17.38 0-8.261-4.777-15.989-12.171-19.688-2.986-1.493-6.364-2.283-9.771-2.283z"
                                                fill="#ffffff" data-original="#000000" class=""></path>
                                        </g>
                                    </g>
                                </g>
                            </g>
                        </svg>
                        <h2>Bank Pertanyaan</h2>
                    </div>
                </div>
            </div>
            <div class="col-md-3">
                <div class="cards">
                    <div class="face face1">
                        <div class="content">
                            <span class="stars"></span>
                            <h2 class="java text-brown">Visualisasi Dashboard</h2>
                            <p class="java">Lihat visualisasi dasbor untuk hasil survei yang telah Anda bagikan.</p>
                        </div>
                    </div>
                    <div class="face face2">
                        <svg xmlns="http://www.w3.org/2000/svg" version="1.1" xmlns:xlink="http://www.w3.org/1999/xlink"
                            xmlns:svgjs="http://svgjs.com/svgjs" width="50px" x="0" y="0" viewBox="0 0 66 66"
                            style="enable-background:new 0 0 512 512" xml:space="preserve" class="">
                            <g>
                                <g>
                                    <path
                                        d="m59.92 1.13h-53.84c-2.68 0-4.86 2.18-4.86 4.86v40.89c0 2.68 2.18 4.86 4.86 4.86h17.95l-2.41 5.95h-3.47c-1.98 0-3.59 1.61-3.59 3.59s1.61 3.59 3.59 3.59h29.68c1.98 0 3.59-1.61 3.59-3.59s-1.61-3.59-3.59-3.59h-3.47l-2.41-5.95h17.95c2.68 0 4.86-2.18 4.86-4.86v-40.89c.02-2.68-2.16-4.86-4.84-4.86zm-10.23 60.15c0 1.02-.83 1.84-1.84 1.84h-29.69c-1.02 0-1.84-.83-1.84-1.84s.83-1.84 1.84-1.84h29.68c1.02 0 1.85.83 1.85 1.84zm-7.2-3.59h-18.98l2.41-5.95h14.15zm20.54-10.81c0 1.72-1.4 3.11-3.11 3.11h-53.84c-1.72 0-3.11-1.4-3.11-3.11v-40.89c0-1.72 1.4-3.11 3.11-3.11h53.83c1.72 0 3.11 1.4 3.11 3.11v40.89z"
                                        fill="#ffffff" data-original="#000000" class=""></path>
                                    <path
                                        d="m59.92 5.11h-53.84c-.48 0-.88.39-.88.88v34.9c0 .48.39.88.88.88h53.83c.48 0 .88-.39.88-.88v-34.9c0-.49-.39-.88-.87-.88zm-.88 34.91h-52.08v-33.16h52.08z"
                                        fill="#ffffff" data-original="#000000" class=""></path>
                                    <circle cx="33" cy="45.3" r="1" fill="#ffffff" data-original="#000000" class="">
                                    </circle>
                                    <path
                                        d="m19.46 29.61c5.65 0 10.25-4.6 10.25-10.25s-4.6-10.25-10.25-10.25-10.25 4.6-10.25 10.26c0 5.65 4.6 10.24 10.25 10.24zm0-1.75c-2.03 0-3.89-.72-5.35-1.91l5.72-5.72h8.09c-.44 4.29-4.07 7.63-8.46 7.63zm8.46-9.37h-7.58v-7.58c3.99.41 7.17 3.59 7.58 7.58zm-9.33-7.58v8.09l-5.72 5.72c-1.19-1.46-1.91-3.33-1.91-5.35 0-4.4 3.35-8.02 7.63-8.46z"
                                        fill="#ffffff" data-original="#000000" class=""></path>
                                    <path
                                        d="m32.51 37.76h4.8c.48 0 .88-.39.88-.88v-9.15c0-.48-.39-.88-.88-.88h-4.8c-.48 0-.88.39-.88.88v9.15c0 .49.39.88.88.88zm.87-9.15h3.05v7.4h-3.05z"
                                        fill="#ffffff" data-original="#000000" class=""></path>
                                    <path
                                        d="m46.11 20.86h-4.8c-.48 0-.88.39-.88.88v15.15c0 .48.39.88.88.88h4.8c.48 0 .88-.39.88-.88v-15.16c-.01-.48-.4-.87-.88-.87zm-.88 15.15h-3.05v-13.4h3.05z"
                                        fill="#ffffff" data-original="#000000" class=""></path>
                                    <path
                                        d="m54.91 29.87h-4.8c-.48 0-.88.39-.88.88v6.15c0 .48.39.88.88.88h4.8c.48 0 .88-.39.88-.88v-6.15c0-.49-.39-.88-.88-.88zm-.87 6.14h-3.05v-4.4h3.05z"
                                        fill="#ffffff" data-original="#000000" class=""></path>
                                    <path
                                        d="m10.09 33.56h15.91c.48 0 .88-.39.88-.88s-.39-.88-.88-.88h-15.91c-.48 0-.88.39-.88.88s.39.88.88.88z"
                                        fill="#ffffff" data-original="#000000" class=""></path>
                                    <path
                                        d="m10.09 37.76h9.37c.48 0 .88-.39.88-.88s-.39-.88-.88-.88h-9.37c-.48 0-.88.39-.88.88s.39.88.88.88z"
                                        fill="#ffffff" data-original="#000000" class=""></path>
                                    <path
                                        d="m32.51 22.61c.22 0 .45-.08.62-.26l8.41-8.41 7.95 7.95c.34.34.9.34 1.24 0l4.7-4.7v.65c0 .48.39.88.88.88s.88-.39.88-.88v-2.91c0-.48-.39-.88-.88-.88h-2.91c-.48 0-.88.39-.88.88s.39.88.88.88h.95l-4.23 4.23-7.95-7.95c-.34-.34-.9-.34-1.24 0l-9.03 9.03c-.34.34-.34.9 0 1.24.16.16.38.25.61.25z"
                                        fill="#ffffff" data-original="#000000" class=""></path>
                                </g>
                            </g>
                        </svg>
                        <br>
                        <h2>Visualisasi Dashboard</h2>
                    </div>
                </div>
            </div>
        </div>

        <div class="pt-5 mb-5 justify-content-center row mobile">
            <div class="col-md-12 text-center">
                <h2>
                    Fungsi Dari Survey
                    <hr style="width: 100px; border: 3px solid #eca869; border-radius: 50px" class="mx-auto">
                </h2>
            </div>

            <div class="col-md-3 mt-3">
                <div class="row justify-content-center align-items-center">
                    <div class="col-8 text-center">
                        <p class="rounded-pill bg-brown px-3 py-1" style="font-size: 16px; font-weight: bold">Reward
                            Responden</p>
                        <p style="font-size: 14px">Anda bisa memberikan reward bagi responden yang telah mengisi survei
                            Anda dengan mudah dan cepat.</p>
                    </div>
                    <div class="col text-start">
                        <svg xmlns="http://www.w3.org/2000/svg" version="1.1" xmlns:xlink="http://www.w3.org/1999/xlink"
                            xmlns:svgjs="http://svgjs.com/svgjs" width="95px" x="0" y="0" viewBox="0 0 512 512.001"
                            style="enable-background:new 0 0 512 512" xml:space="preserve" class="">
                            <g>
                                <path
                                    d="m233.035156 460.980469v-10.304688c0-.246093-.011718-.496093-.035156-.746093-4.003906-40.019532-21.5-66.460938-50.578125-76.453126l-27.570313-9.566406-49.550781-42.066406c-7.542969-6.40625-18.945312-5.949219-25.960937 1.027344-6.300782 6.277344-7.371094 15.84375-3.03125 23.269531l-3.988282-1.835937-22.425781-27.03125-10.15625-25.828126c-3.699219-9.410156-13.921875-14.796874-23.773437-12.523437-5.761719 1.324219-10.675782 5.066406-13.488282 10.269531-2.8125 5.199219-3.25 11.363282-1.203124 16.910156l11.578124 31.394532c.191407.523437.445313 1.023437.75 1.488281l30.972657 47.671875c.175781.269531.367187.523438.574219.769531 10.738281 12.636719 24.289062 22.949219 39.1875 29.828125 12.296874 5.675782 30.003906 14.691406 41.742187 27.378906 3.015625 3.257813 5.882813 7.144532 7.886719 17.558594-4.402344 2.597656-7.363282 7.390625-7.363282 12.859375v22.03125c0 8.226563 6.691407 14.917969 14.917969 14.917969h86.550781c8.226563 0 14.917969-6.691406 14.917969-14.917969v-22.03125c0-6.484375-4.160156-12.019531-9.953125-14.070312zm-142.414062-57.347657c-12.792969-5.90625-24.449219-14.738281-33.738282-25.5625l-30.25-46.554687-11.289062-30.605469c-.757812-2.058594-.121094-3.757812.328125-4.585937s1.519531-2.292969 3.660156-2.785157c2.667969-.617187 5.445313.84375 6.449219 3.394532l10.59375 26.945312c.292969.738282.699219 1.429688 1.207031 2.042969l24.28125 29.269531c.714844.863282 1.617188 1.554688 2.636719 2.023438l37.191406 17.121094 19.609375 22.308593c1.480469 1.683594 3.554688 2.546875 5.636719 2.546875 1.757812 0 3.523438-.617187 4.949219-1.867187 3.109375-2.734375 3.414062-7.472657.679687-10.585938l-20.660156-23.5c-.046875-.054687-.097656-.109375-.148438-.160156l-21.980468-23.792969c-1.507813-1.660156-1.441406-4.199218.148437-5.78125 1.527344-1.523437 4.019531-1.625 5.667969-.226562l50.621094 42.976562c.707031.597656 1.519531 1.0625 2.394531 1.363282l28.917969 10.035156c23.398437 8.042968 37.027344 29.375 40.511718 63.402344v9.078124h-69.203124c-2.464844-13.292968-6.523438-20.042968-11.746094-25.691406-13.550782-14.640625-33.011719-24.59375-46.46875-30.808594zm137.367187 93.367188h-86.386719v-21.867188h86.386719zm0 0"
                                    fill="#61481c" data-original="#000000" class=""></path>
                                <path
                                    d="m494.542969 289.191406c-2.808594-5.203125-7.726563-8.945312-13.488281-10.269531-9.851563-2.273437-20.074219 3.113281-23.773438 12.523437l-10.15625 25.824219-22.425781 27.035157-3.96875 1.828124c4.324219-7.398437 3.257812-16.980468-3.050781-23.261718-7.011719-6.980469-18.417969-7.433594-25.957032-1.03125l-49.554687 42.066406-27.546875 9.5625c-29.101563 10-46.597656 36.4375-50.597656 76.460938-.027344.246093-.039063.496093-.039063.746093v10.304688c-5.792969 2.050781-9.953125 7.582031-9.953125 14.070312v22.027344c0 8.226563 6.695312 14.917969 14.917969 14.917969h86.550781c8.226562 0 14.917969-6.691406 14.917969-14.917969v-22.027344c0-5.472656-2.960938-10.265625-7.359375-12.863281 2-10.414062 4.871094-14.300781 7.882812-17.558594 11.738282-12.6875 29.445313-21.699218 41.742188-27.378906 14.898437-6.878906 28.453125-17.191406 39.1875-29.828125.210937-.242187.402344-.5.574218-.769531l30.972657-47.667969c.304687-.46875.558593-.96875.75-1.492187l11.578125-31.390626c2.046875-5.546874 1.609375-11.710937-1.203125-16.910156zm-139.125 207.808594h-86.386719v-21.867188h86.386719zm126.257812-196.089844-11.289062 30.605469-30.25 46.554687c-9.289063 10.824219-20.945313 19.65625-33.738281 25.5625-13.457032 6.214844-32.914063 16.164063-46.464844 30.8125-5.222656 5.644532-9.28125 12.394532-11.746094 25.6875h-69.203125v-9.074218c3.484375-34.03125 17.113281-55.363282 40.535156-63.410156l28.890625-10.027344c.878906-.304688 1.691406-.769532 2.398438-1.367188l50.621094-42.976562c1.648437-1.394532 4.136718-1.296875 5.667968.226562 1.589844 1.582032 1.65625 4.125.199219 5.730469l-22.03125 23.839844c-.046875.054687-.097656.109375-.148437.164062l-20.660157 23.5c-2.734375 3.113281-2.429687 7.851563.683594 10.585938 1.421875 1.253906 3.1875 1.867187 4.949219 1.867187 2.078125 0 4.152344-.859375 5.632812-2.546875l19.609375-22.304687 37.191407-17.121094c1.019531-.472656 1.921874-1.160156 2.636718-2.027344l24.28125-29.269531c.507813-.609375.914063-1.300781 1.207032-2.042969l10.59375-26.941406c1.003906-2.554688 3.777343-4.015625 6.449218-3.398438 2.140625.492188 3.210938 1.957032 3.660156 2.785157.449219.828125 1.082032 2.527343.324219 4.585937zm0 0"
                                    fill="#61481c" data-original="#000000" class=""></path>
                                <path
                                    d="m373.320312 134.125 16.421876 13.109375-5.578126 20.257813c-1.183593 4.296874.382813 8.761718 3.984376 11.382812 1.871093 1.355469 4.042968 2.039062 6.222656 2.039062 2.019531 0 4.042968-.585937 5.832031-1.765624l17.542969-11.5625 17.546875 11.5625c3.71875 2.449218 8.449219 2.34375 12.054687-.273438 3.601563-2.617188 5.167969-7.085938 3.984375-11.382812l-5.578125-20.257813 16.421875-13.113281c3.480469-2.777344 4.839844-7.308594 3.464844-11.546875-1.378906-4.234375-5.144531-7.101563-9.589844-7.308594l-20.992187-.953125-7.398438-19.671875c-1.566406-4.167969-5.457031-6.863281-9.914062-6.863281-4.453125 0-8.34375 2.695312-9.914063 6.863281l-7.394531 19.671875-20.992188.953125c-4.449218.203125-8.214843 3.074219-9.589843 7.308594-1.375 4.238281-.015625 8.769531 3.464843 11.550781zm32.742188-5.054688c2.996094-.136718 5.621094-2.042968 6.675781-4.851562l5.011719-13.320312 5.007812 13.320312c1.054688 2.808594 3.683594 4.714844 6.679688 4.851562l14.214844.648438-11.121094 8.878906c-2.34375 1.871094-3.347656 4.960938-2.550781 7.851563l3.777343 13.722656-11.882812-7.832031c-1.25-.824219-2.6875-1.238282-4.125-1.238282s-2.878906.414063-4.128906 1.238282l-11.882813 7.832031 3.777344-13.722656c.796875-2.890625-.207031-5.980469-2.550781-7.851563l-11.121094-8.878906zm0 0"
                                    fill="#61481c" data-original="#000000" class=""></path>
                                <path
                                    d="m34.84375 134.125 16.421875 13.109375-5.578125 20.261719c-1.183594 4.292968.382812 8.761718 3.984375 11.378906 3.605469 2.617188 8.335937 2.722656 12.054687.273438l17.546876-11.5625 17.542968 11.5625c1.789063 1.179687 3.8125 1.765624 5.832032 1.765624 2.179687 0 4.351562-.683593 6.222656-2.039062 3.605468-2.621094 5.167968-7.085938 3.984375-11.382812l-5.578125-20.257813 16.421875-13.109375c3.480469-2.78125 4.839843-7.3125 3.464843-11.550781-1.375-4.234375-5.140624-7.101563-9.589843-7.308594l-20.992188-.953125-7.394531-19.671875c-1.570312-4.167969-5.460938-6.863281-9.914062-6.863281-4.457032 0-8.347657 2.695312-9.914063 6.863281l-7.394531 19.671875-20.992188.953125c-4.453125.203125-8.214844 3.074219-9.589844 7.308594-1.378906 4.238281-.019531 8.769531 3.460938 11.550781zm32.738281-5.054688c2.996094-.136718 5.625-2.042968 6.679688-4.851562l5.011719-13.320312 5.007812 13.320312c1.054688 2.808594 3.683594 4.714844 6.679688 4.851562l14.214843.648438-11.121093 8.878906c-2.34375 1.871094-3.347657 4.960938-2.550782 7.851563l3.777344 13.722656-11.882812-7.832031c-2.503907-1.652344-5.75-1.652344-8.253907 0l-11.882812 7.832031 3.777343-13.722656c.796876-2.890625-.207031-5.980469-2.550781-7.851563l-11.121093-8.878906zm0 0"
                                    fill="#61481c" data-original="#000000" class=""></path>
                                <path
                                    d="m204.082031 46.34375 16.421875 13.113281-5.578125 20.257813c-1.183593 4.296875.382813 8.761718 3.984375 11.378906 3.601563 2.617188 8.335938 2.726562 12.054688.273438l17.546875-11.5625 17.542969 11.566406c1.789062 1.179687 3.8125 1.765625 5.832031 1.765625 2.179687 0 4.351562-.683594 6.222656-2.042969 3.605469-2.617188 5.167969-7.082031 3.984375-11.378906l-5.578125-20.257813 16.421875-13.113281c3.480469-2.777344 4.839844-7.3125 3.464844-11.546875-1.375-4.238281-5.140625-7.105469-9.589844-7.308594l-20.992188-.957031-7.398437-19.667969c-1.566406-4.167969-5.457031-6.863281-9.910156-6.863281-4.457031 0-8.347657 2.695312-9.914063 6.863281l-7.398437 19.667969-20.992188.957031c-4.449219.203125-8.214843 3.070313-9.589843 7.308594-1.375 4.234375-.015626 8.769531 3.464843 11.546875zm32.742188-5.050781c2.996093-.136719 5.621093-2.046875 6.675781-4.855469l5.011719-13.320312 5.007812 13.320312c1.054688 2.808594 3.683594 4.71875 6.679688 4.855469l14.214843.644531-11.121093 8.882812c-2.34375 1.871094-3.347657 4.957032-2.550781 7.851563l3.777343 13.722656-11.878906-7.835937c-1.253906-.824219-2.691406-1.238282-4.128906-1.238282s-2.875.414063-4.128907 1.238282l-11.882812 7.835937 3.777344-13.722656c.796875-2.894531-.207032-5.980469-2.550782-7.851563l-11.121093-8.882812zm0 0"
                                    fill="#61481c" data-original="#000000" class=""></path>
                                <path
                                    d="m375.097656 238.234375c0-69.800781-56.789062-126.589844-126.585937-126.589844-69.800781 0-126.589844 56.789063-126.589844 126.589844 0 69.796875 56.789063 126.585937 126.589844 126.585937 69.796875 0 126.585937-56.789062 126.585937-126.585937zm-126.585937 111.585937c-61.53125 0-111.589844-50.058593-111.589844-111.585937 0-61.53125 50.058594-111.589844 111.589844-111.589844 61.527343 0 111.585937 50.058594 111.585937 111.589844 0 61.527344-50.058594 111.585937-111.585937 111.585937zm0 0"
                                    fill="#61481c" data-original="#000000" class=""></path>
                                <path
                                    d="m156.542969 230.636719c4.082031.667969 7.945312-2.101563 8.617187-6.191407 3.203125-19.523437 13.292969-37.417968 28.410156-50.375 15.277344-13.09375 34.789063-20.304687 54.941407-20.304687 20.148437 0 39.664062 7.210937 54.9375 20.304687 15.117187 12.957032 25.207031 30.851563 28.410156 50.375.605469 3.679688 3.785156 6.289063 7.390625 6.289063.40625 0 .816406-.03125 1.226562-.101563 4.085938-.667968 6.855469-4.527343 6.183594-8.613281-3.773437-23.011719-15.652344-44.082031-33.445312-59.335937-17.996094-15.425782-40.976563-23.917969-64.703125-23.917969-23.730469 0-46.710938 8.492187-64.703125 23.917969-17.796875 15.253906-29.675782 36.324218-33.449219 59.335937-.671875 4.085938 2.097656 7.945313 6.183594 8.617188zm0 0"
                                    fill="#61481c" data-original="#000000" class=""></path>
                                <path
                                    d="m340.476562 245.832031c-4.085937-.671875-7.945312 2.097657-8.617187 6.1875-3.203125 19.527344-13.292969 37.417969-28.410156 50.375-15.277344 13.09375-34.789063 20.308594-54.9375 20.308594-20.152344 0-39.664063-7.214844-54.941407-20.308594-15.117187-12.957031-25.207031-30.847656-28.410156-50.375-.671875-4.085937-4.53125-6.859375-8.617187-6.1875-4.085938.671875-6.855469 4.527344-6.183594 8.613281 3.773437 23.011719 15.652344 44.082032 33.449219 59.335938 17.992187 15.425781 40.972656 23.917969 64.703125 23.917969 23.726562 0 46.707031-8.492188 64.703125-23.917969 17.792968-15.253906 29.671875-36.324219 33.445312-59.335938.671875-4.085937-2.097656-7.941406-6.183594-8.613281zm0 0"
                                    fill="#61481c" data-original="#000000" class=""></path>
                                <path
                                    d="m249.828125 284.203125c1.886719 0 3.769531-1.382813 3.769531-2.890625v-4.269531c13.566406-1.886719 22.863282-10.429688 22.863282-25.878907 0-17.085937-11.933594-22.613281-22.863282-26.632812v-25.503906c8.792969.628906 12.058594 4.648437 15.074219 4.648437 3.769531 0 5.527344-4.773437 5.527344-7.160156 0-6.15625-12.058594-8.792969-20.601563-9.046875v-3.390625c0-1.507813-1.886718-2.890625-3.769531-2.890625-2.136719 0-3.644531 1.382812-3.644531 2.890625v3.644531c-11.933594 1.253906-23.867188 7.535156-23.867188 23.363282 0 16.082031 12.5625 20.605468 23.867188 24.625v29.519531c-12.8125-1.003907-16.207032-9.796875-20.351563-9.796875-3.140625 0-5.777343 4.144531-5.777343 7.160156 0 6.15625 10.550781 14.570312 26.128906 14.824219v3.894531c.003906 1.507812 1.511718 2.890625 3.644531 2.890625zm3.015625-45.726563c6.15625 2.636719 11.054688 6.15625 11.054688 14.070313 0 7.15625-4.269532 11.179687-11.054688 12.433594zm-17.964844-28.644531c0-6.53125 5.023438-9.671875 12.0625-10.554687v22.867187c-6.660156-2.511719-12.0625-5.152343-12.0625-12.3125zm0 0"
                                    fill="#61481c" data-original="#000000" class=""></path>
                            </g>
                        </svg>
                    </div>
                </div>
            </div>

            <div class="col-md-3 mt-3">
                <div class="row justify-content-center align-items-center">
                    <div class="col text-end">
                        <svg xmlns="http://www.w3.org/2000/svg" version="1.1" xmlns:xlink="http://www.w3.org/1999/xlink"
                            xmlns:svgjs="http://svgjs.com/svgjs" width="95px" x="0" y="0" viewBox="0 0 64 64"
                            style="enable-background:new 0 0 512 512" xml:space="preserve" class="">
                            <g>
                                <path
                                    d="M22,23H15V16a1,1,0,0,0-1-1,9,9,0,1,0,9,9A1,1,0,0,0,22,23Zm-9-5.929v6.515l-4.6,4.6A6.985,6.985,0,0,1,13,17.071ZM14,31a6.953,6.953,0,0,1-4.184-1.4l4.6-4.6h6.515A7.011,7.011,0,0,1,14,31Z"
                                    fill="#61481c" data-original="#000000" class=""></path>
                                <path
                                    d="M18,21h8a1,1,0,0,0,1-1,9.011,9.011,0,0,0-9-9,1,1,0,0,0-1,1v8A1,1,0,0,0,18,21Zm1-7.929A7.018,7.018,0,0,1,24.929,19H19Z"
                                    fill="#61481c" data-original="#000000" class=""></path>
                                <path
                                    d="M39,26A13,13,0,1,0,52,39,13.015,13.015,0,0,0,39,26Zm-5,9V29.214a10.974,10.974,0,0,1,2-.788V35Zm12,2v2H32V37Zm-4-2V28.426a10.974,10.974,0,0,1,2,.788V35Zm-2-6.949V35H38V28.051c.33-.03.662-.051,1-.051S39.67,28.021,40,28.051Zm-8,2.47V35H31a1,1,0,0,0-1,1v4a1,1,0,0,0,1,1H47a1,1,0,0,0,1-1V36a1,1,0,0,0-1-1H46V30.521A10.948,10.948,0,0,1,49.236,43H28.764A10.948,10.948,0,0,1,32,30.521ZM39,50a11,11,0,0,1-9.208-5H48.208A11,11,0,0,1,39,50Z"
                                    fill="#61481c" data-original="#000000" class=""></path>
                                <path
                                    d="M61.671,55.257l-4.326-4.326A2,2,0,0,0,57,48.586l-2.545-2.545A16.883,16.883,0,0,0,53,29.378V4a3,3,0,0,0-3-3H4A3,3,0,0,0,1,4V42a3,3,0,0,0,3,3H23.114a16.934,16.934,0,0,0,22.927,9.455L48.586,57a1.972,1.972,0,0,0,2.345.345l4.326,4.326a4.535,4.535,0,1,0,6.414-6.414ZM4,3H50a1,1,0,0,1,1,1V7H3V4A1,1,0,0,1,4,3ZM4,43a1,1,0,0,1-1-1V9H51V26.971a17.056,17.056,0,0,0-5-3.445V12a1,1,0,0,0-1-1H41a1,1,0,0,0-1,1V22.051c-.333-.02-.662-.051-1-.051s-.667.031-1,.051V18a1,1,0,0,0-1-1H33a1,1,0,0,0-1,1v5.526A16.882,16.882,0,0,0,22.5,43ZM44,22.753a16.817,16.817,0,0,0-2-.472V13h2Zm-8-.472a16.817,16.817,0,0,0-2,.472V19h2ZM24,39A15,15,0,1,1,39,54,15.017,15.017,0,0,1,24,39ZM47.882,53.468a17.1,17.1,0,0,0,5.586-5.586L55.586,50,50,55.586ZM58.464,61a2.52,2.52,0,0,1-1.793-.743L52.414,56,56,52.414l4.257,4.257A2.536,2.536,0,0,1,58.464,61Z"
                                    fill="#61481c" data-original="#000000" class=""></path>
                                <path
                                    d="M18,35H6a1,1,0,0,0-1,1v4a1,1,0,0,0,1,1H18a1,1,0,0,0,1-1V36A1,1,0,0,0,18,35Zm-1,4H7V37H17Z"
                                    fill="#61481c" data-original="#000000" class=""></path>
                            </g>
                        </svg>
                    </div>
                    <div class="col-8 text-center">
                        <p class="rounded-pill bg-brown px-3 py-1" style="font-size: 16px; font-weight: bold">Survey
                            Template Market</p>
                        <p style="font-size: 14px">Pilih dengan harga terjangkau untuk semua kualitas template survei di
                            Indo Survei.</p>
                    </div>
                </div>
            </div>

            <div class="col-md-3 mt-3">
                <div class="row justify-content-center align-items-center">
                    <div class="col-8 text-center">
                        <p class="rounded-pill bg-brown px-3 py-1" style="font-size: 16px; font-weight: bold">Bank
                            Pertanyaan</p>
                        <p style="font-size: 14px">Ada Bank Soal yang bisa ditambahkan ke survei Anda dengan cepat &
                            efisien.</p>
                    </div>
                    <div class="col text-start">
                        <svg xmlns="http://www.w3.org/2000/svg" version="1.1" xmlns:xlink="http://www.w3.org/1999/xlink"
                            xmlns:svgjs="http://svgjs.com/svgjs" width="95px" x="0" y="0" viewBox="0 0 64 64"
                            style="enable-background:new 0 0 512 512" xml:space="preserve" class="">
                            <g>
                                <g>
                                    <g id="LWPOLYLINE">
                                        <g>
                                            <path
                                                d="m31.321 38.904c-.006 0-.012 0-.018 0-1.104-.01-1.992-.913-1.982-2.018l.009-1c0-.057.003-.115.009-.172.277-2.909 2.208-4.692 3.761-6.125.526-.486 1.024-.945 1.448-1.424.517-.583 1.267-1.777.478-3.218-.909-1.664-3.129-2.135-4.854-1.74-1.802.412-2.466 1.951-2.702 2.831-.286 1.067-1.383 1.7-2.45 1.415-1.067-.286-1.7-1.383-1.415-2.45.797-2.977 2.865-5.052 5.673-5.695 3.779-.863 7.585.664 9.256 3.719 1.391 2.544 1.01 5.529-.995 7.791-.557.628-1.152 1.178-1.728 1.709-1.435 1.325-2.332 2.211-2.484 3.49l-.008.903c-.008 1.101-.902 1.984-1.998 1.984z"
                                                fill="#61481c" data-original="#000000" class=""></path>
                                        </g>
                                    </g>
                                    <g id="POINT">
                                        <g>
                                            <path
                                                d="m31.32 44.9c-.52 0-1.04-.21-1.41-.58-.38-.37-.59-.89-.59-1.42 0-.52.21-1.04.59-1.41.74-.74 2.08-.74 2.82 0 .38.37.59.89.59 1.41 0 .53-.21 1.04-.58 1.42-.38.37-.89.58-1.42.58z"
                                                fill="#61481c" data-original="#000000" class=""></path>
                                        </g>
                                    </g>
                                    <g id="CIRCLE_2_">
                                        <g>
                                            <path
                                                d="m31.923 57.997c-2.275 0-4.53-.296-6.703-.881-9.364-2.519-16.696-10.276-18.679-19.763-2.021-9.665 1.709-19.825 9.504-25.884 4.506-3.503 10.189-5.433 16.003-5.433 4.025 0 8.022.936 11.56 2.705 8.737 4.371 14.382 13.503 14.382 23.266 0 7.966-3.759 15.644-10.055 20.538-4.522 3.516-10.209 5.452-16.012 5.452zm.126-47.961c-4.929 0-9.741 1.63-13.548 4.591-6.597 5.128-9.755 13.727-8.044 21.907 1.678 8.025 7.881 14.588 15.803 16.719 1.834.493 3.74.744 5.663.744h.001c4.918 0 9.732-1.637 13.557-4.61 5.329-4.142 8.51-10.639 8.51-17.38 0-8.261-4.777-15.989-12.171-19.688-2.986-1.493-6.364-2.283-9.771-2.283z"
                                                fill="#61481c" data-original="#000000" class=""></path>
                                        </g>
                                    </g>
                                </g>
                            </g>
                        </svg>
                    </div>
                </div>
            </div>

            <div class="col-md-3 mt-3">
                <div class="row justify-content-center align-items-center">
                    <div class="col text-end">
                        <svg xmlns="http://www.w3.org/2000/svg" version="1.1" xmlns:xlink="http://www.w3.org/1999/xlink"
                            xmlns:svgjs="http://svgjs.com/svgjs" width="95px" x="0" y="0" viewBox="0 0 66 66"
                            style="enable-background:new 0 0 512 512" xml:space="preserve" class="">
                            <g>
                                <g>
                                    <path
                                        d="m59.92 1.13h-53.84c-2.68 0-4.86 2.18-4.86 4.86v40.89c0 2.68 2.18 4.86 4.86 4.86h17.95l-2.41 5.95h-3.47c-1.98 0-3.59 1.61-3.59 3.59s1.61 3.59 3.59 3.59h29.68c1.98 0 3.59-1.61 3.59-3.59s-1.61-3.59-3.59-3.59h-3.47l-2.41-5.95h17.95c2.68 0 4.86-2.18 4.86-4.86v-40.89c.02-2.68-2.16-4.86-4.84-4.86zm-10.23 60.15c0 1.02-.83 1.84-1.84 1.84h-29.69c-1.02 0-1.84-.83-1.84-1.84s.83-1.84 1.84-1.84h29.68c1.02 0 1.85.83 1.85 1.84zm-7.2-3.59h-18.98l2.41-5.95h14.15zm20.54-10.81c0 1.72-1.4 3.11-3.11 3.11h-53.84c-1.72 0-3.11-1.4-3.11-3.11v-40.89c0-1.72 1.4-3.11 3.11-3.11h53.83c1.72 0 3.11 1.4 3.11 3.11v40.89z"
                                        fill="#61481c" data-original="#000000" class=""></path>
                                    <path
                                        d="m59.92 5.11h-53.84c-.48 0-.88.39-.88.88v34.9c0 .48.39.88.88.88h53.83c.48 0 .88-.39.88-.88v-34.9c0-.49-.39-.88-.87-.88zm-.88 34.91h-52.08v-33.16h52.08z"
                                        fill="#61481c" data-original="#000000" class=""></path>
                                    <circle cx="33" cy="45.3" r="1" fill="#61481c" data-original="#000000" class="">
                                    </circle>
                                    <path
                                        d="m19.46 29.61c5.65 0 10.25-4.6 10.25-10.25s-4.6-10.25-10.25-10.25-10.25 4.6-10.25 10.26c0 5.65 4.6 10.24 10.25 10.24zm0-1.75c-2.03 0-3.89-.72-5.35-1.91l5.72-5.72h8.09c-.44 4.29-4.07 7.63-8.46 7.63zm8.46-9.37h-7.58v-7.58c3.99.41 7.17 3.59 7.58 7.58zm-9.33-7.58v8.09l-5.72 5.72c-1.19-1.46-1.91-3.33-1.91-5.35 0-4.4 3.35-8.02 7.63-8.46z"
                                        fill="#61481c" data-original="#000000" class=""></path>
                                    <path
                                        d="m32.51 37.76h4.8c.48 0 .88-.39.88-.88v-9.15c0-.48-.39-.88-.88-.88h-4.8c-.48 0-.88.39-.88.88v9.15c0 .49.39.88.88.88zm.87-9.15h3.05v7.4h-3.05z"
                                        fill="#61481c" data-original="#000000" class=""></path>
                                    <path
                                        d="m46.11 20.86h-4.8c-.48 0-.88.39-.88.88v15.15c0 .48.39.88.88.88h4.8c.48 0 .88-.39.88-.88v-15.16c-.01-.48-.4-.87-.88-.87zm-.88 15.15h-3.05v-13.4h3.05z"
                                        fill="#61481c" data-original="#000000" class=""></path>
                                    <path
                                        d="m54.91 29.87h-4.8c-.48 0-.88.39-.88.88v6.15c0 .48.39.88.88.88h4.8c.48 0 .88-.39.88-.88v-6.15c0-.49-.39-.88-.88-.88zm-.87 6.14h-3.05v-4.4h3.05z"
                                        fill="#61481c" data-original="#000000" class=""></path>
                                    <path
                                        d="m10.09 33.56h15.91c.48 0 .88-.39.88-.88s-.39-.88-.88-.88h-15.91c-.48 0-.88.39-.88.88s.39.88.88.88z"
                                        fill="#61481c" data-original="#000000" class=""></path>
                                    <path
                                        d="m10.09 37.76h9.37c.48 0 .88-.39.88-.88s-.39-.88-.88-.88h-9.37c-.48 0-.88.39-.88.88s.39.88.88.88z"
                                        fill="#61481c" data-original="#000000" class=""></path>
                                    <path
                                        d="m32.51 22.61c.22 0 .45-.08.62-.26l8.41-8.41 7.95 7.95c.34.34.9.34 1.24 0l4.7-4.7v.65c0 .48.39.88.88.88s.88-.39.88-.88v-2.91c0-.48-.39-.88-.88-.88h-2.91c-.48 0-.88.39-.88.88s.39.88.88.88h.95l-4.23 4.23-7.95-7.95c-.34-.34-.9-.34-1.24 0l-9.03 9.03c-.34.34-.34.9 0 1.24.16.16.38.25.61.25z"
                                        fill="#61481c" data-original="#000000" class=""></path>
                                </g>
                            </g>
                        </svg>
                    </div>
                    <div class="col-8 text-center">
                        <p class="rounded-pill bg-brown px-3 py-1" style="font-size: 16px; font-weight: bold">
                            Visualisasi Dashboard</p>
                        <p style="font-size: 14px">Lihat visualisasi dasbor untuk hasil survei yang telah Anda bagikan.
                        </p>
                    </div>
                </div>
            </div>

        </div>
    </div>
</section>

<section>
    <div class="container pt-5 pb-5">
        <h2 class="mb-4 text-center">
            Fungsi Dari Survey
            <hr style="width: 100px; border: 3px solid #eca869; border-radius: 50px" class="mx-auto">
        </h2>
        <div class="row mt-5 mb-4 align-items-center">
            <div class="col-md-6 mt-2 mb-2">
                <img style="width: 100%" src="{{url('/')}}/assets/illustration/customer.svg" alt="">
            </div>
            <div class="col-md-6 mt-2 mb-2">
                <h2 class="fw-bold text-warm mb-4">Kenali pelanggan Anda lebih baik</h2>
                <p class="mb-0">Cari tahu seberapa baik kinerja bisnis Anda, dan di mana Anda perlu meningkatkan untuk
                    mencapai atau mempertahankan CX yang hebat.
                </p>
            </div>
            <div class="col-md-6 mt-2 mb-2">
                <h2 class="fw-bold text-warm mb-4">Riset Pasar</h2>
                <p class="mb-0">Perluas kemampuan riset pasar Anda untuk melacak merek Anda dan menguji ide-ide Anda
                    kepada audiens yang Anda targetkan.
                </p>
            </div>
            <div class="col-md-6 mt-2 mb-2">
                <img style="width: 80%" src="{{url('/')}}/assets/illustration/marketing.svg" alt="">
            </div>
        </div>
    </div>
</section>

<section class="sect-bg">
    <div class="container pt-5 pb-5">
        <h2 class="mb-4 text-center">
            Sampel Survey Kami
            <hr style="width: 100px; border: 3px solid #eca869; border-radius: 50px" class="mx-auto">
        </h2>
        <div class="row">
            <div class="col-md-4 mt-2 mb-2">
                <div class="card text-left">
                    <div class="card-header bg-thumb">
                    </div>
                    <div class="card-body">
                        <span class="badge rounded-pill bg-warm mb-1">New!</span>
                        <h4 class="card-title">Lorem ipsum dolor sit amet.</h4>
                        <p class="card-text mb-5">Lorem ipsum dolor sit amet consectetur adipisicing elit. Pariatur,
                            nesciunt!</p>
                        <p class="card-text text-warm"><i class="fa-solid fa-download"></i> 5 Unduhan</p>
                    </div>
                </div>
            </div>
            <div class="col-md-4 mt-2 mb-2">
                <div class="card text-left">
                    <div class="card-header bg-thumb">
                    </div>
                    <div class="card-body">
                        <span class="badge rounded-pill bg-warm mb-1">New!</span>
                        <h4 class="card-title">Lorem ipsum dolor sit amet.</h4>
                        <p class="card-text mb-5">Lorem ipsum dolor sit amet consectetur adipisicing elit. Pariatur,
                            nesciunt!</p>
                        <p class="card-text text-warm"><i class="fa-solid fa-download"></i> 5 Unduhan</p>
                    </div>
                </div>
            </div>
            <div class="col-md-4 mt-2 mb-2">
                <div class="card text-left">
                    <div class="card-header bg-thumb">
                    </div>
                    <div class="card-body">
                        <span class="badge rounded-pill bg-warm mb-1">New!</span>
                        <h4 class="card-title">Lorem ipsum dolor sit amet.</h4>
                        <p class="card-text mb-5">Lorem ipsum dolor sit amet consectetur adipisicing elit. Pariatur,
                            nesciunt!</p>
                        <p class="card-text text-warm"><i class="fa-solid fa-download"></i> 5 Unduhan</p>
                    </div>
                </div>
            </div>
            <div class="col-md-4 mt-2 mb-2">
                <div class="card text-left">
                    <div class="card-header bg-thumb">
                    </div>
                    <div class="card-body">
                        <span class="badge rounded-pill bg-warm mb-1">New!</span>
                        <h4 class="card-title">Lorem ipsum dolor sit amet.</h4>
                        <p class="card-text mb-5">Lorem ipsum dolor sit amet consectetur adipisicing elit. Pariatur,
                            nesciunt!</p>
                        <p class="card-text text-warm"><i class="fa-solid fa-download"></i> 5 Unduhan</p>
                    </div>
                </div>
            </div>
            <div class="col-md-4 mt-2 mb-2">
                <div class="card text-left">
                    <div class="card-header bg-thumb">
                    </div>
                    <div class="card-body">
                        <span class="badge rounded-pill bg-warm mb-1">New!</span>
                        <h4 class="card-title">Lorem ipsum dolor sit amet.</h4>
                        <p class="card-text mb-5">Lorem ipsum dolor sit amet consectetur adipisicing elit. Pariatur,
                            nesciunt!</p>
                        <p class="card-text text-warm"><i class="fa-solid fa-download"></i> 5 Unduhan</p>
                    </div>
                </div>
            </div>
            <div class="col-md-4 mt-2 mb-2">
                <div class="card text-left">
                    <div class="card-header bg-thumb">
                    </div>
                    <div class="card-body">
                        <span class="badge rounded-pill bg-warm mb-1">New!</span>
                        <h4 class="card-title">Lorem ipsum dolor sit amet.</h4>
                        <p class="card-text mb-5">Lorem ipsum dolor sit amet consectetur adipisicing elit. Pariatur,
                            nesciunt!</p>
                        <p class="card-text text-warm"><i class="fa-solid fa-download"></i> 5 Unduhan</p>
                    </div>
                </div>
            </div>
        </div>
    </div>
</section>

<section>
    <div class="container pt-5 pb-5">
        <div class="card" style="background: rgb(107,211,210);
background: linear-gradient(90deg, rgba(107,211,210,1) 0%, rgba(214,0,255,1) 100%); border-radius: 20px">
            <div class="card-body text-center p-5">
                <h1 class="mb-5 text-white">Ingin Mecoba SmartSurvey?</h1>
                <h6 class="mb-5 text-white">Dapatkan platform yang dapat berkembang dan berubah seiring dengan kebutuhan
                    survei Anda dari waktu ke waktu.</h6>
                <button class="btn" style="background: white; color: #000">SIGN UP, IT'S FREE</button>
            </div>
        </div>
    </div>
</section>
{{-- <div class="container">
    <div class="card mt-5 mx-auto card-responsive">
        <div class="card-body">
            <div class="" id="hilang-buat">
               <center>
                <img src="{{url('/')}}/assets/logo/logo.png" alt="">
</center>
<h5 class="card-title mt-3">Buat Survei Dengan Mudah</h5>
<p class="card-text">Anda dapat membuat survei dan membagikan link surveinya pada teman Anda!</p>
<a name="" class="btn btn-success" href="#" role="button" id="btn-buat">Buat Survei</a>
<div class="alert alert-primary my-3" role="alert">
    <strong>Total Survei :</strong> 6258
</div>

<h5 class="card-title mt-3">Tentang Raja Survei</h5>
<p class="card-text">
    Rajasurvei.com didirikan untuk menyajikan data
    jajak pendapat secara terbuka. Kehadiran
    Rajasurvei.com untuk membetikan alternatif survei
    yang mudah. independen dan berintegritas.
    Selamat membuat survei di Rajasurvei.com
</p>
<h5 class="card-title mt-3">Syarat & Ketentuan</h5>
<p class="card-text">
    Dengan membuat dan mengikuti survei ini, Anda menerima
    Syarat dan ketentuan
</p>
<p class="text-center mb-0 mt-5" style="font-size: 12px">About Us</p>
</div>

<div class="d-none" id="muncul-buat">
    <div class="row align-items-center">
        <div class="col-2">
            <button class="btn" id="kembali"><i class="fa-solid fa-arrow-left"></i></button>
        </div>
        <div class="col-8">
            <h3 class="fw-bold mb-0 text-center">Indosurvei</h3>
        </div>
        <div class="col-2">
        </div>
    </div>
    <h5 class="card-title mt-3 mb-3">Buat Survei Anda</h5>
    <form action="">
        <div class="mb-3">
            <label for="nama" class="form-label">Nama</label>
            <input type="text" class="form-control" name="nama" id="nama" placeholder="Masukkan nama Anda">
        </div>
        <div class="mb-3">
            <label for="no-hp" class="form-label">No Hp</label>
            <input type="number" class="form-control" name="no-hp" id="no-hp" placeholder="Masukkan no. hp Anda">
        </div>
        <div class="mb-3">
            <label for="email" class="form-label">Email</label>
            <input type="email" class="form-control" name="email" id="email" placeholder="Masukkan email Anda">
        </div>
        <div class="mb-3">
            <label for="judul" class="form-label">Judul Survei</label>
            <input type="text" class="form-control" name="judul" id="judul" placeholder="Masukkan judul survei Anda"
                aria-describedby="judulHelp">
            <div id="judulHelp" class="form-text">contoh: Pengkhianatan Indonesia</div>
        </div>
        <div class="mb-3">
            <label for="pertanyaan" class="form-label">Pertanyaan Anda</label>
            <textarea class="form-control" name="pertanyaan" id="pertanyaan" rows="3"
                placeholder="Masukkan pertanyaan Anda"></textarea>
        </div>
        <div class="mb-3">
            <div class="mb-3">
                <label for="gambar" class="form-label">Masukkan Gambar</label>
                <input type="file" class="form-control" name="gambar" id="gambar" placeholder="Masukkan gambar Anda">
            </div>
        </div>
        <div class="mb-3">
            <button type="submit" class="btn btn-success">Submit</button>
        </div>
    </form>
</div>
</div>
</div>

</div> --}}

@endsection
